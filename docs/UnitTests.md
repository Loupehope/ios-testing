...
Quick/Nimble - это [BDD фреймворк](https://en.wikipedia.org/wiki/Behavior-driven_development), который позволяет описать тест в категориях - given, when, then (describe, context, it - в контексте фреймворка). С фреймворком меняется именно синтаксис тестов, сама логика по формированию моков, протоколов и т.д. не меняется. Детали фреймворка можно посмотреть в самом репозитории проекта.
```swift
import XCTest
import UIKit
